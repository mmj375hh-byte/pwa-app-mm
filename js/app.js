...
function openChartModal() {
    ...
    // モーダルが開き終わるのを少し待ってから、chart.js側の描画関数を呼び出す
...
function closeChartModal() {
    if (chartModalInstance) {
        ...
        document.querySelector('button[onclick="openChartModal()"]').focus(); // フォーカスを戻す
    }
}
...
function closeHistoryModal() {
    if (historyModalInstance) {
        ...
        document.querySelector('button[onclick="openHistoryModal()"]').focus(); // フォーカスを戻す
    }
}
...
async function deleteRecord(id) {
    if (confirm('この記録を削除してもよろしいですか？')) {
        ...
        // もし全履歴モーダルが開いていたら、モーダル内のリストや重量フィルターも連動して再集計する
...
// メイン画面側：直近5件制限の簡易履歴リスト生成（日付の降順ソート対応）
async function updateHistoryList() {
    ...
    // 同日内のセットも含めて、完全に日付の新しい順(降順)・IDの降順にソート
...
// 重量と日付のフィルター条件による、全履歴リストのリアルタイム絞り込み（日付の降順対応）
...
async function changePeriod(period) {
    ...
    await updateChart(); // chart.js側の関数
}
